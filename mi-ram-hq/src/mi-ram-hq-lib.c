...
char * obtener_proxima_tarea_paginacion(uint32_t tripulante_tid)
{
	...
	if(!patota){
		...
		//Como maneja discordiador el error de obtener_proxima_tarea?
		return RESPUESTA_FAIL;
	}
...
void sighandlerCompactacion(int signum) {
...
void mover_tripulante_mapa (char simbolo,direccion dir){
	switch(dir) {
		int err;
		case ABAJO:
			err = item_desplazar(nivel, simbolo, 0, -1);
		break;
		...
		case IZQUIERDA:
			err = item_desplazar(nivel, simbolo, -1, 0);
		break;
